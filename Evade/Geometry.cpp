...
namespace Evade
{
    float Geometry::AngleBetween(const Vector& v1, const Vector& v2)
    {
        ...
        float len_a = v1.LengthSquared();
        float len_b = v2.LengthSquared();
        float len = std::sqrtf(len_a * len_b);
        return std::acosf(dot / len) * 180.0f / M_PI;
    }
        ...
        const Vector& p2, const Vector& vel, float speed, float offset)
    {
        ...
        float b = 2.0f * (vel.DotProduct(dir) - multi);
