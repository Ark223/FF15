...
namespace Evade
{
    ...
    template<typename T>
    Linq<T>::Linq(std::initializer_list<T> init_list) : data(init_list) {}
    ...
    {
        return *std::find_if(data.begin(), data.end(), predicate);
    }
    ...
    {
        if (index == -1) index = 0;
        if (count == -1) count = data.size() - index;

        auto end = data.begin() + index + count;
        auto it = std::find(data.begin() + index, end, item);
        return (it != end) ? std::distance(data.begin(), it) : -1;
    }
    ...
    {
        return *std::find_if(data.rbegin(), data.rend(), predicate);
    }
    ...
    int Linq<T>::LastIndexOf(T item, int index, int count) const
    {
        ...
        auto rbegin = data.rbegin() + (data.size() - 1 - index);
        auto it = std::find(rbegin, rbegin + count, item);

        if (it == rbegin + count) return -1;
        return std::distance(data.begin(), it.base()) - 1;
    }
    ...
    template<typename T>
    void Linq<T>::RemoveAt(int index)
    {
        data.erase(data.begin() + index);
    }

    template<typename T>
    void Linq<T>::RemoveRange(int index, int count)
    {
