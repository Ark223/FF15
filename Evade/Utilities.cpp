...
namespace Evade
{
    ...
    bool Utilities::EvadeToSpot(const Vector& spot) const
    {
        ...
        // No need to repeat an action if we follow the path
        const Vector& dest = this->api->GetPathEndPos(this->hero);
        ...
        {
            case 0: offset.x = 10.0f; break;
            case 1: offset.x = -10.0f; break;
            case 2: offset.y = 10.0f; break;
            case 3: offset.y = -10.0f; break;
        }
    ...
    Vector Utilities::Execute(Solution& solution) const
    {
        ...
        GameObject* target = solution.Target;
    ...
    {
        std::pair<float, size_t> result = { FLT_INF, 0 };
        ...
        Path& path, float max_range, float dash_range, float delta, bool fixed)
    {
        ...
        bool infinite = std::isinf(path.Speed);
        ...
        while ((angle -= step) > -0.01f)
        {
            ...
            Vector rotated = vertical.Rotate(MAX(0.0f, phi));
    ...
    {
        int size = poly.Size();
        float closest_distance = FLT_INF;
    ...
    Solution Utilities::FindSolution(Linq<Skillshot*> skillshots, bool any_dynamic)
    {
        ...
        {
            return skillshot->Get<int>("DangerLevel");
        });
        ...
        bool apply_short = any_dynamic || skillshots.Any([&](Skillshot* skillshot)
        {
            ...
            return name == "XerathRMissileWrapper" || accel != 0.0f;
        });
        ...
        for (size_t index = 0; index <= size; index++)
        {
            ...
            spots.RemoveAll([&](const Vector& spot)
            {
                ...
                {
                    auto type = skillshot->Get<SkillshotType>("SkillshotType");
            ...
            if (threats.Any()) spots.RemoveAll([&](const Vector& spot)
            {
                ...
                return angle_a > 0.5f * M_PI && angle_b < 0.5f * M_PI;
            });
