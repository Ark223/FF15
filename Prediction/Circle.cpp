...
namespace Prediction
{
    ...
    int Circle::CountCovered(const Solution& circle)
    {
        this->points.Count([&](const Vector& point)
        {
            return this->IsInside(circle, point);
        });
    }

    bool Circle::EraseFarthest(const Solution& circle)
    {
    ...
    Solution Circle::FindMEC()
    {
        Linq<Vector> points = this->points;
        return this->Welzl(points);
    }
